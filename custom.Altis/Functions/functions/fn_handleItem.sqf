...
if(((primaryWeaponMagazine player) select 0) in _badMagazines && !((primaryWeapon player) in _badWeapons)) then {
player setAmmo [primaryWeapon player, 0];
["Du kannst dieses Magazin nicht in diese Waffe laden", "Magazin verboten"] spawn life_fnc_message;
};