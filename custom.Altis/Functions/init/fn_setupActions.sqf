...
life_actions = life_actions + [player addAction["Ausrauben",life_fnc_robAction,"",0,false,false,"",'!isNull cursorObject && player distance cursorObject < 3.5 && isPlayer cursorObject && (side cursorObject) != independent && (((lifeState cursorObject) in ["INCAPACITATED"]) || (animationState cursorObject) in ["amovpercmstpsnonwnondnon_amovpercmstpssurwnondnon"])']];
life_actions = life_actions + [player addAction["Selbstbehandlung",life_fnc_healAction,false,1,false,true,"",'((["firstaidkit"] call life_fnc_getItemValue) > 0) && (((damage player) > (["heal"] call life_fnc_getSkillAdvantage)) || ((player getHitPointDamage "hitLegs") >= 0.5)) && {(!(player getVariable["restrained",false]))} && {isNull objectParent player}']];
...
player addAction ["", {}, "", 10, false, true, "DefaultAction", "life_blockshooting"];