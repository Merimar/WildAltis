...
{if([_x, "uniform"] call life_fnc_canAddItem) then {player addItemToUniform _x;};} forEach _uItems;
{if([_x, "uniform"] call life_fnc_canAddItem) then {(uniformContainer player) addItemCargoGlobal [_x,1];};} forEach _uMags;
{if([_x, "vest"] call life_fnc_canAddItem) then {player addItemToVest _x;};} forEach (_vItems);
{if([_x, "vest"] call life_fnc_canAddItem) then {(vestContainer player) addItemCargoGlobal [_x,1];};} forEach _vMags;
{if([_x, "backpack"] call life_fnc_canAddItem) then {player addItemToBackpack _x;};} forEach _bItems;
{if([_x, "backpack"] call life_fnc_canAddItem) then {(backpackContainer player) addItemCargoGlobal [_x,1];};} forEach _bMags;
