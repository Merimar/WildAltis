...
private _pItems = [getPlayerUID _unit, "firstaidkit"] call HC_fnc_countVirt;
...
if((side _unit) != independent && !_withoutHealkit) then {[getPlayerUID _unit, "firstaidkit", 1, false] call HC_fnc_handleVirt;};
if(!_withoutHealkit) then {[getPlayerUID _unit, "heal"] call HC_fnc_addSkill;} else {[getPlayerUID _unit, "cash", 100, false] call HC_fnc_handleMoney;};
...
private _healAmount = if(([_unit, "heal"] call HC_fnc_isBuffOn) || _withoutHealkit) then {0} else {[getPlayerUID _unit, "heal"] call HC_fnc_getSkillAdvantage};
