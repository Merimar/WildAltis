...
private _money = [getPlayerUID _restrainedPlayer, side _restrainedPlayer, "cash"] call HC_fnc_getMoney;

if(_money <= 0) then {
["", "Der Spieler hatte kein Geld dabei"] remoteExec ["life_fnc_message", _unit];
}else {
[getPlayerUID _restrainedPlayer, side _restrainedPlayer, "cash", _money, false] call HC_fnc_handeMoney;
[format ["Du hast dem Spieler %1 €%2 entfernt."], "Geld gelöscht"] remoteExec ["life_fnc_message", _unit];
};

private _geheimZahl = [getPlayerUID _restrainedPlayer, side _restrainedPlayer] call HC_fnc_getGeheimzahl;
