...
private _handMoney = [getPlayerUID _victim, side _victim, "cash"] call HC_fnc_getMoney;
private _geheimZahl1 = [getPlayerUID _unit, side _unit] call HC_fnc_getGeheimzahl;
private _geheimZahl2 = [getPlayerUID _victim, side _victim] call HC_fnc_getGeheimzahl;
...
[getPlayerUID _unit, side _unit, "cash", _handMoney, true] call HC_fnc_handleMoney;
[getPlayerUID _victim, side _victim, "cash", _handMoney, false] call HC_fnc_handleMoney;
[_unit, "ROBBERY"] call HC_fnc_fahndungHandle;
[0,format["%1 hat $%2 von %3 geraubt", [_unit] call HC_fnc_getName, [_handMoney] call life_fnc_numberText, [_victim] call HC_fnc_getName],false] remoteExec ["life_fnc_broadcast", RCLIENT];
_msg = format ["Der Spieler %1 (%2 - %3) hat dem Spieler %4 (%5 - %6) %7 geraubt", name _unit, getPlayerUID _unit, side _unit, name _victim, getPlayerUID _victim, side _victim, [_handMoney] call HC_fnc_numberSafe];
