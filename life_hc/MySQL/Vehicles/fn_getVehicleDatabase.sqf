...
private _query = format["SELECT v.vuid, l.classname, o.type, v.active, c.color, m.material, v.insurance, v.impounded, v.abo FROM vehicles v LEFT JOIN life_classnames l ON v.classname_id = l.id LEFT JOIN life_object_types o ON l.type_id = o.id LEFT JOIN life_vehicle_materials m ON v.material_id = m.id LEFT JOIN life_vehicle_colors c ON v.color_id = c.id WHERE v.owner_id = '%1' AND v.side_id = '%2' AND deleted_at IS NOT NULL", _ownerID, _sideID];
