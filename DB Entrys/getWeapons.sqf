...
private _finalText = "";
private _config = "true" configClasses (missionConfigFile >> "CfgItemShop");
private _classnames = [];
{
private _uniformArray = getArray (_x >> "uniform");
private _headgearArray = getArray (_x >> "headgear");
private _gogglesArray = getArray (_x >> "goggles");
private _vestArray = getArray (_x >> "vest");
private _backpackArray = getArray (_x >> "backpack");
private _weaponsArray = getArray (_x >> "weapons");
private _itemArray = getArray (_x >> "items");

{
private _classname = _x select 0;
if(!(_classname in _classnames)) then {
_classnames pushBack _classname;
private _displayName = ([_classname] call GET_VEHICLE_INFO) select 1;
...
_finalText = _finalText + format ["INSERT INTO life_classnames (classname, type_id, slug) VALUES ('%1', '%2', '%3');", _classname, 4, _newDisplayName];
};
}forEach (_uniformArray + _headgearArray + _gogglesArray + _vestArray + _backpackArray + _weaponsArray + _itemArray);
}forEach _config;
