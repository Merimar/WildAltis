...
private _currentLastDay = _lastDay select _dayLast;
...
if(_rewardIndex isEqualTo -1) then {_rewards pushBack [_rewardClass, false];}else {_rewards pushBack [_rewardClass, (_input select _rewardIndex select 1) isEqualTo 1];};
}forEach _allRewards;
